...
def store_digits(n):
    ...
    while a // 10 != 0:
        b += 1
        a //= 10
    return Link(a, store_digits(n - a * (10 ** b)))
...
class VirFib():
    ...
    def next(self):
        "*** YOUR CODE HERE ***"
        nxt = VirFib(self.lst + self.value)
        nxt.lst = self.value
        return nxt
...
def bst_min(t):
    """ Returns the min label in Tree t. """
    if t.is_leaf():
        return t.label
    if len(t.branches) == 1:
        return bst_min(t.branches[0])
    return min(bst_min(t.branches[0]), bst_min(t.branches[1]))


def bst_max(t):
    """ Returns the max label in Tree t. """
    if t.is_leaf():
        return t.label
    if len(t.branches) == 1:
        return bst_max(t.branches[0])
    return max(bst_max(t.branches[0]), bst_max(t.branches[1]))
