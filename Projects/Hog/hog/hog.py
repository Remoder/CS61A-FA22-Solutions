...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            #score0 = update(strategy0(score0, score1), score0, score1)
        else:
            score1 = update(strategy1(score1, score0), score1, score0, dice)
            #score1 = update(strategy1(score1, score0), score1, score0) 
        who = 1 - who
    # END PROBLEM 5
...
def always_roll(n):
    ...
    3
    """
    assert n >= 0 and n <= 10, 'The number must be between 0 and 10'
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def f(score0, score1):
        return n
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while k < 10:
        k = k + 1
        average_function = make_averaged(roll_dice, total_samples)
        tmp = average_function(k, dice)
        if tmp > maxx:
            maxx = tmp
            ans = k
    return ans
...
def final_strategy(score, opponent_score, goal=GOAL):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    closing_value = 15
    # BEGIN PROBLEM 12
    if goal - score <= closing_value:
        return tail_strategy(score, opponent_score, threshold=closing_value/2, num_rolls=2)

    if score < opponent_score:
        if tail_points(opponent_score) > 10:
            return 0
        return min(int((goal-score) * 1.5 // 100) + 2, 8)
    else:
        return min(int((goal-score) * 0.8 // 100), 5)
    # END PROBLEM 12
