...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        tmp, cnt = self.place, 0
        while (tmp.entrance) and ((not tmp.bees) or cnt < self.lower_bound):
            tmp = tmp.entrance
            cnt += 1

        if (not tmp.is_hive) and (self.lower_bound <= cnt <= self.upper_bound):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        damage = self.damage + amount if self.health <= 0 else amount
...
# The WallAnt class
class WallAnt(Ant):
    """ WallAnt with large health can protect other ants. """
    name = 'Wall'
    food_cost = 4
    implemented = True
...
# The HungryAnt Class
class HungryAnt(Ant):
    """ HungryAnt can kill a bee immediately but needs time to chew. """
    name = "Hungry"
    food_cost = 4
    chewing_turns = 3
    implemented = True
    ...
    def action(self, gamestate):
        if self.turns_to_chew:
            self.turns_to_chew = (self.turns_to_chew + 1) % (self.chewing_turns + 1)
...
class TankAnt(ContainerAnt):
    """ TankAnt can not only protect but also attack. """
    name = 'Tank'
    damage = 1
    food_cost = 6
    implemented = True
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        while tmp:
            ant1 = tmp.ant
            ant2 = tmp.ant.ant_contained if ant1 and ant1.is_container else None
            if ant1 and ant1 not in self.doubled_ants:
                ant1.double()
                self.doubled_ants.append(ant1)
            if ant2 and ant2 not in self.doubled_ants:
                ant2.double()
                self.doubled_ants.append(ant2)
            tmp = tmp.exit
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def inner_func(gamestate):
            if target.slow_turn:
                if not target.slow_turn % 2:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        tmp = self.place
        dic, dis = {}, 0
        while tmp and not tmp.is_hive:
            ant1 = tmp.ant
            ant2 = ant1.ant_contained if ant1 and ant1.is_container else None
            if ant1 and ant1 is not self:
                dic.update({ant1: dis})
            if ant2 and ant2 is not self:
                dic.update({ant2: dis})
            for bee in tmp.bees:
                dic.update({bee: dis})
            tmp, dis = tmp.entrance, dis + 1
        return dic
        # END Problem Optional 2

    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        truly_damage = self.damage - distance * 0.25 - self.insects_shot * 0.0625
        return max(0, truly_damage)
        # END Problem Optional 2
