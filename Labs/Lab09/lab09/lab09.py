...
class VendingMachine:
    ...
    """
    "*** YOUR CODE HERE ***"
    def __init__(self, good_name, good_price, inventory=0, fund=0):
    ...
    def vend(self):
        response = ''
        if self.inventory:
            if self.fund < self.good_price:
                response = 'Please update your balance with ${0} more funds.'.format(self.good_price-self.fund)
            elif self.fund == self.good_price:
                response = 'Here is your {0}.'.format(self.good_name)
                self.fund, self.inventory = 0, self.inventory - 1
            else:
                response = 'Here is your {0} and ${1} change.'.format(self.good_name, self.fund-self.good_price)
                self.fund, self.inventory = 0, self.inventory - 1
        else:
            response = 'Nothing left to vend. Please restock.'
        return response
    ...
    def add_funds(self, x):
        ...
        if self.inventory:
            self.fund += x
            response = 'Current balance: ${0}'.format(self.fund)
        else:
            response = 'Nothing left to vend. Please restock. Here is your ${0}.'.format(x)
        return response

    def restock(self, x):
        self.inventory += x
        return 'Current {0} stock: {1}'.format(self.good_name, self.inventory)  
...
def long_paths(t, n):
    ...
    for sub_tree in t.branches:
        for sub_part in long_paths(sub_tree, max(0, n-1)):
            res.append([t.label] + sub_part)
    return [path for path in res if len(path) > n]
...
def reverse_other(t):
    ...
    reversed_lst = []
    for sub_tree in t.branches:
        reversed_lst.append(sub_tree.label)
        for sub_sub_tree in sub_tree.branches:
            reverse_other(sub_sub_tree)
    reversed_lst.reverse()
    for sub_tree in t.branches:
        sub_tree.label, reversed_lst = reversed_lst[0], reversed_lst[1:]
