...
def closer_city(lat, lon, city_a, city_b):
    ...
    if distance(goal_city, city_a) < distance(goal_city, city_b):
...
def sprout_leaves(t, leaves):
    ...
    new_branches = []
    for sub_tree in branches(t):
        new_branches += [sprout_leaves(sub_tree, leaves)]
...
def add_trees(t1, t2):
    ...
    len_t1 = len(branches_t1)
    len_t2 = len(branches_t2)
    if len_t1 < len_t2:
        branches_t1 += [[0]] * (len_t2 - len_t1)
    elif len_t1 > len_t2:
        branches_t2 += [[0]] * (len_t1 - len_t2)
    new_label = label(t1) + label(t2)
    for idx in range(max(len_t1, len_t2)):
        sub_t1 = branches_t1[idx]
        sub_t2 = branches_t2[idx]
        new_branches += [add_trees(sub_t1, sub_t2)]
